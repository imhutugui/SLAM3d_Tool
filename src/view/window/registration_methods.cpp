...
pcl::Registration<pcl::PointXYZI, pcl::PointXYZI>::Ptr RegistrationMethods::method() const {
  ...
  switch(registration_method) {
    case 0: {
      auto icp = std::make_shared<pcl::IterativeClosestPoint<pcl::PointXYZI, pcl::PointXYZI>>();
      registration = icp;
    } break;
    ...
    case 1: {
      auto gicp = std::make_shared<pcl::GeneralizedIterativeClosestPoint<pcl::PointXYZI, pcl::PointXYZI>>();
      registration = gicp;
    } break;

    case 2: {
      auto ndt = std::make_shared<pcl::NormalDistributionsTransform<pcl::PointXYZI, pcl::PointXYZI>>();
      ndt->setResolution(registration_resolution);
      registration = ndt;
    } break;

    case 3: {
      auto gicp = std::make_shared<pclomp::GeneralizedIterativeClosestPoint<pcl::PointXYZI, pcl::PointXYZI>>();
      registration = gicp;
    } break;
