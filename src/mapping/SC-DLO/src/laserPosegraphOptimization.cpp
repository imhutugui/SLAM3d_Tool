...
void MapOptimization::loadParams(std::string yaml_file) {
    YAML::Node node;
    try
    {
    ...
    YAML::Node pgoNode = node["dlo"]["alaserPGO"];
    if (pgoNode["save_map"]) save_map = pgoNode["save_map"].as<bool>();
    else save_map = false;
    if (pgoNode["save_directory"]) save_directory = pgoNode["save_directory"].as<std::string>();
    else save_directory = "/tmp/dump/";
    if (pgoNode["keyframe_meter_gap"]) keyframeMeterGap = pgoNode["keyframe_meter_gap"].as<double>();
    else keyframeMeterGap = 2.0;
    if (pgoNode["keyframe_deg_gap"]) keyframeDegGap = pgoNode["keyframe_deg_gap"].as<double>();
    else keyframeDegGap = 10.0;
    keyframeRadGap = deg2rad(keyframeDegGap);
    if (pgoNode["sc_dist_thres"]) scDistThres = pgoNode["sc_dist_thres"].as<double>();
    else scDistThres = 0.2;
    if (pgoNode["sc_max_radius"]) scMaximumRadius = pgoNode["sc_max_radius"].as<double>();
    else scMaximumRadius = 80.0;
    double filter_size = 0.4;
    if (pgoNode["scan_filter_size"]) filter_size = pgoNode["scan_filter_size"].as<double>();
    double mapVizFilterSize = 0.4;
    if (pgoNode["map_vis_filter_size"]) mapVizFilterSize = pgoNode["mapviz_filter_size"].as<int>();
...
void MapOptimization::startLoopDetection() {
    exit_flag = false;
    loadParams("/media/south/storage/projects/lio_ws/src/SLAM3d_Tool/src/mapping/SC-DLO/cfg/dlo_single.yaml");
    ...
    if (pgTimeSaveStream.is_open()) {
        std::cout << "pgTimeSaveStream is already open, closing it first." << std::endl;
        pgTimeSaveStream.close();
    }
    pgTimeSaveStream.open(save_directory + "times.txt", std::fstream::out);
    pgTimeSaveStream.precision(std::numeric_limits<double>::max_digits10);
  std::thread loop(&MapOptimization::process_lcd, this);
  loop.detach();
  std::thread posegraph_slam(&MapOptimization::process_pg, this);
  posegraph_slam.detach();
  std::thread icp_caculation(&MapOptimization::process_icp, this);
  icp_caculation.detach();
  std::thread isam_update(&MapOptimization::process_isam, this);
  isam_update.detach();
}

void MapOptimization::stopLoopDetection() {exit_flag = true;}

void MapOptimization::saveSCD(std::string fileName, Eigen::MatrixXd matrix, std::string delimiter = " ") {
  // delimiter: ", " or " " etc.
...
void MapOptimization::saveGTSAMgraphG2oFormat(const gtsam::Values &_estimates)
{
...
void MapOptimization::saveOdometryVerticesKITTIformat(std::string _filename)
{
...
void MapOptimization::laserOdometryHandler(const nav_msgs::Odometry::ConstPtr &_laserOdometry)
{
    mBuf.lock();
...
void MapOptimization::laserCloudFullResHandler(const sensor_msgs::PointCloud2ConstPtr &_laserCloudFullRes)
{
    mBuf.lock();
...
void MapOptimization::gpsHandler(const sensor_msgs::NavSatFix::ConstPtr &_gps)
{
    if (useGPS)
    {
        mBuf.lock();
...
void MapOptimization::updatePoses(void)
{
    mKF.lock();
...
void MapOptimization::runISAM2opt(void)
{
...
void MapOptimization::loopFindNearKeyframesCloud(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key, const int &submap_size, const int &root_idx)
{
...
std::optional<gtsam::Pose3> MapOptimization::doICPVirtualRelative(int _loop_kf_idx, int _curr_kf_idx)
{
...
void MapOptimization::process_pg()
{
    while (!exit_flag)
    {
        while (!odometryBuf.empty() && !fullResBuf.empty())
        {
            ...
            std::string keyframe_directory = (boost::format("%s/%06d") % pgScansDirectory % curr_node_idx_str).str();
            boost::filesystem::create_directories(keyframe_directory);
            pcl::io::savePCDFileBinary(keyframe_directory + "/cloud.pcd", *thisKeyFrame); // scan
            std::ofstream data_ofs(keyframe_directory + "/data");
            ros::Time stamp(timeLaser);
            gtsam::Pose3 kf_pose = Pose6DtoGTSAMPose3(keyframePoses.at(curr_node_idx));
            data_ofs << "stamp " << stamp.sec << " " << stamp.nsec << "\n";
            data_ofs << "estimate \n" << kf_pose.matrix() << "\n";
            data_ofs << "odom \n" << kf_pose.matrix() << "\n";
            data_ofs << "accum_distance -1" << "\n";
            data_ofs << "id " << curr_node_idx << "\n";
            data_ofs.close();
...
// info: 执行闭环检测
void MapOptimization::performSCLoopClosure(void)
{
    // info: 关键帧太少时先跳过
...
void MapOptimization::process_lcd(void)
{
    // info: 回环检测的频率
    ...
    {
        // rate.sleep();
        std::this_thread::sleep_for(std::chrono::milliseconds(int(1000 / loopClosureFrequency)));
        performSCLoopClosure();
        // performRSLoopClosure(); // TODO
    }
} // process_lcd

void MapOptimization::process_icp(void)
{
    while (!exit_flag)
    {
        while (!scLoopICPBuf.empty())
        {
            ...
            {
                dbg("Too many loop clousre candidates to be ICPed is waiting ... Do process_lcd less frequently (adjust loopClosureFrequency)");
            }
...
void MapOptimization::process_isam(void)
{
    float hz = 1;
    // ros::Rate rate(hz);
    while (!exit_flag)
