...
//   this->publish_timer = this->nh.createTimer(ros::Duration(0.01), &dlio::OdomNode::publishPose, this);
  ...
  this->original_scan = pcl::PointCloud<PointT>::ConstPtr (std::make_shared<const pcl::PointCloud<PointT>>());
  this->deskewed_scan = pcl::PointCloud<PointT>::ConstPtr (std::make_shared<const pcl::PointCloud<PointT>>());
  this->current_scan = pcl::PointCloud<PointT>::ConstPtr (std::make_shared<const pcl::PointCloud<PointT>>());
  this->submap_cloud = pcl::PointCloud<PointT>::ConstPtr (std::make_shared<const pcl::PointCloud<PointT>>());
  ...
  pcl::Registration<PointT, PointT>::KdTreeReciprocalPtr temp;
...
void dlio::OdomNode::getParams(std::string yaml_file) {
    ...
    {
        std::cerr << "Unable to open " << yaml_file << ", " << e.what() << std::endl;
        throw e;
    }

  // Version
  this->version_ = "0.0.0";
  if (node["dlio"]["version"]) this->version_ = node["dlio"]["version"].as<std::string>();

//   ros::param::param<std::string>("~dlio/version", this->version_, "0.0.0");

  // Frames
  auto frameNode = node["dlio"]["frames"];
  this->odom_frame = "odom";
  this->baselink_frame = "base_link";
  this->lidar_frame = "lidar";
  this->imu_frame = "imu";
...
//   ros::param::param<std::vector<float>>("~dlio/imu/intrinsics/accel/sm", imu_sm, imu_sm_default);
  ...
  this->gicp_transformation_ep_ = 0.005;
  this->gicp_rotation_ep_ = 0.005;
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointT>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
...
void dlio::OdomNode::publishCloud(pcl::PointCloud<PointT>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {

  if (this->wait_until_move_) {
    if (this->length_traversed < 0.1) { return; }
  }

  pcl::PointCloud<PointT>::Ptr deskewed_scan_t_ (std::make_shared<pcl::PointCloud<PointT>>());
...
//   this->deskewed_pub.publish(deskewed_ros);
  ...
  odom_msg.pose.pose.position.x = this->state.p[0];
  odom_msg.pose.pose.position.y = this->state.p[1];
  odom_msg.pose.pose.position.z = this->state.p[2];
  odom_msg.pose.pose.orientation.w = this->state.q.w();
  odom_msg.pose.pose.orientation.x = this->state.q.x();
  odom_msg.pose.pose.orientation.y = this->state.q.y();
  odom_msg.pose.pose.orientation.z = this->state.q.z();
  nav_msgs::Odometry::ConstPtr odom_ptr(new nav_msgs::Odometry(odom_msg));
  sensor_msgs::PointCloud2ConstPtr cloud_ptr(new sensor_msgs::PointCloud2(deskewed_ros));
  keyframe_signal.emit(odom_ptr, cloud_ptr);
}

void dlio::OdomNode::publishKeyframe(std::pair<std::pair<Eigen::Vector3f, Eigen::Quaternionf>, pcl::PointCloud<PointT>::ConstPtr> kf, ros::Time timestamp) {
...
void dlio::OdomNode::getScanFromROS(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  pcl::PointCloud<PointT>::Ptr original_scan_ (std::make_shared<pcl::PointCloud<PointT>>());
...
void dlio::OdomNode::preprocessPoints() {
  ...
  if (this->deskew_) {
    ...
    pcl::PointCloud<PointT>::Ptr deskewed_scan_ (std::make_shared<pcl::PointCloud<PointT>>());
  ...
  // Voxel Grid Filter
  if (this->vf_use_) {
    pcl::PointCloud<PointT>::Ptr current_scan_
      (std::make_shared<pcl::PointCloud<PointT>>(*this->deskewed_scan));
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  std::function<bool(boost::range::index_value<PointT&, std::ptrdiff_t>,
                     boost::range::index_value<PointT&, std::ptrdiff_t>)> point_time_neq;
  std::function<double(boost::range::index_value<PointT&, std::ptrdiff_t>)> extract_point_time;

  if (this->sensor == dlio::SensorType::OUSTER) {

    point_time_cmp = [](const PointT& p1, const PointT& p2)
      { return p1.t < p2.t; };
    point_time_neq = [](boost::range::index_value<PointT&, std::ptrdiff_t> p1,
                        boost::range::index_value<PointT&, std::ptrdiff_t> p2)
      { return p1.value().t != p2.value().t; };
    extract_point_time = [&sweep_ref_time](boost::range::index_value<PointT&, std::ptrdiff_t> pt)
  ...
  if (frames.size() != timestamps.size()) {
    std::cerr << "Bad time sync between LiDAR and IMU!";
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  if (this->current_scan->points.size() <= this->gicp_min_num_points_) {
    std::cerr << "Low number of points in the cloud!";
    return;
  }
  ...
  pcl::PointCloud<PointT>::ConstPtr published_cloud;
...
void dlio::OdomNode::getNextPose() {
  ...
  pcl::PointCloud<PointT>::Ptr aligned (std::make_shared<pcl::PointCloud<PointT>>());
...
void dlio::OdomNode::computeConvexHull() {
  ...
  pcl::PointCloud<PointT>::Ptr cloud =
    pcl::PointCloud<PointT>::Ptr (std::make_shared<pcl::PointCloud<PointT>>());
...
void dlio::OdomNode::buildSubmap(State vehicle_state) {
  ...
  if (this->submap_kf_idx_curr != this->submap_kf_idx_prev){
    ...
    pcl::PointCloud<PointT>::Ptr submap_cloud_ (std::make_shared<pcl::PointCloud<PointT>>());
...
void dlio::OdomNode::buildKeyframesAndSubmap(State vehicle_state) {
  ...
  for (int i = this->num_processed_keyframes; i < this->keyframes.size(); i++) {
    pcl::PointCloud<PointT>::ConstPtr raw_keyframe = this->keyframes[i].second;
    ...
    pcl::PointCloud<PointT>::Ptr transformed_keyframe (std::make_shared<pcl::PointCloud<PointT>>());
