...
public:
  ...
  KDBindings::Signal<const sensor_msgs::PointCloud2ConstPtr&> loop_cloud_signal;
  KDBindings::Signal<const nav_msgs::Odometry::ConstPtr&> odom_signal;
  KDBindings::Signal<const nav_msgs::Odometry::ConstPtr&, const sensor_msgs::PointCloud2ConstPtr&> keyframe_signal;
...
private:
  ...
  void getParams(std::string yaml_file);



  void publishPose();

  void publishToROS(pcl::PointCloud<PointT>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud);
  void publishCloud(pcl::PointCloud<PointT>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud);
  void publishKeyframe(std::pair<std::pair<Eigen::Vector3f, Eigen::Quaternionf>,
                       pcl::PointCloud<PointT>::ConstPtr> kf, ros::Time timestamp);
...
//   ros::Publisher deskewed_pub;
  ...
  std::vector<std::pair<std::pair<Eigen::Vector3f, Eigen::Quaternionf>,
                        pcl::PointCloud<PointT>::ConstPtr>> keyframes;
  ...
  // Point Clouds
  pcl::PointCloud<PointT>::ConstPtr original_scan;
  pcl::PointCloud<PointT>::ConstPtr deskewed_scan;
  pcl::PointCloud<PointT>::ConstPtr current_scan;

  // Keyframes
  pcl::PointCloud<PointT>::ConstPtr keyframe_cloud;
  ...
  // Submap
  pcl::PointCloud<PointT>::ConstPtr submap_cloud;
  std::shared_ptr<const nano_gicp::CovarianceList> submap_normals;
  std::shared_ptr<const nanoflann::KdTreeFLANN<PointT>> submap_kdtree;
  ...
  nano_gicp::NanoGICP<PointT, PointT> gicp_temp;
